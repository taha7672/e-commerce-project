                ...
                <div class="widget-content widget-content-area br-8">
                    <div class="row mb-4">
                        ...
                        <div class="col-4 text-end">
                            <a class="btn btn-success" href="{{route('admin.tags.create')}}">{{__('pages.add_new')}}</a>
                        </div>
                    </div>
                            ...
                            @if(count($tags)>0)
                                @foreach ($tags as $index => $tag)
                                    <tr>
                                        ...
                                        <td>
                                        @php
                                            $imagePath = public_path($tag->image);
                                        @endphp

                                        @if($tag->image && file_exists($imagePath))
                                            <img 
                                                src="{{ asset($tag->image) }}" 
                                                alt="{{ $tag->name }}" 
                                                width="100" 
                                        ...
                                        <td>{{$tag->is_active? __('pages.active') : __('pages.inactive') }}</td>
                                        <td>
                                            <a class="btn btn-info btn-icon" href="{{route('admin.tags.edit',$tag->id)}}">
                                                <i class="fa-solid fa-pen-to-square"></i>
                                            </a>
                                            <form action="{{route('admin.tags.destroy',$tag->id)}}" class="d-inline" method="post">
                                                @csrf
                                                @method('DELETE')
