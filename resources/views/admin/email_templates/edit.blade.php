    ...
    <div class="row layout-top-spacing">
        <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
            <div class="widget-content widget-content-area br-8">
                <div class="row mb-4">
                    <div class="col-8">
                        <h4>{{__('pages.edit_email_template')}}</h4>
                    </div>
                </div>
					<div class="container"> 
						...
						<form action="{{ route('admin.email-templates.update', $emailTemplate->id) }}" method="POST">
							...
							<div class="form-group mt-2">
								...
								<textarea name="body" class="form-control wysiwyg" rows="10" required>{{ $emailTemplate->body }}</textarea>
							</div>
							<button type="submit" class="btn btn-primary">{{__('pages.update')}}</button>
							<a href="{{ route('admin.email-templates.index') }}" class="btn btn-secondary">{{__('pages.cancel')}}</a>
						</form>
					</div>
            </div>
        </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal" onclick="$('#shortcodesModal').modal('hide')">Close</button> 
      </div>
    </div>
  </div>
</div>
