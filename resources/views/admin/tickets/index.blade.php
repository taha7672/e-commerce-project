        ...
        <div class="row layout-top-spacing">
            <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
                <div class="widget-content widget-content-area br-8">
                    <div class="row mb-4">
                        <div class="col-8">
                            <h4>{{__('pages.tickets')}}</h4>
                        </div>
                        <div class="col-4 text-end">
                            <a class="btn btn-success" href="{{route('admin.tickets.create')}}">{{__('pages.add_new')}}</a>
                        </div>
                    </div>
                    <table id="categories" class="table dt-table-hover" style="width:100%">
                        <thead>
                            <tr>
                                <th>{{ __('tables.no') }}</th>
                                <th>{{ __('tables.subject') }}</th>
                                <th>{{ __('tables.status') }}</th>
                                <th>{{ __('tables.priority') }}</th>
                                <th>{{ __('tables.assigned_to') }}</th>
                        ...
                        @if(count($tickets)>0)
                                @foreach ($tickets as $ticket)
                                    <tr>
                                        ...
                                        <td>{{ucwords($ticket->prioritys?->name) }}</td> 
                                        <td>{{ucwords($ticket->admin->name)}}</td>
                                        <td>
                                            <a class="btn btn-info btn-icon" href="{{route('admin.tickets.edit',$ticket->id)}}">
                                            ...
                                            <a class="btn btn-primary btn-icon m-2" href="{{route('admin.tickets.comment',$ticket->id)}}">
