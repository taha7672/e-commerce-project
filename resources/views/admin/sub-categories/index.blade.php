        ...
        <div class="row layout-top-spacing">
            <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
                <div class="widget-content widget-content-area br-8">
                    <div class="row mb-4">
                        <div class="col-8">
                            <h4>{{__('pages.sub_categories')}}</h4>
                        </div>
                        <div class="col-4 text-end">
                            <a class="btn btn-success" href="{{route('admin.sub-categories.create')}}">{{__('pages.add_new')}}</a>
                        </div>
                    </div>
                            ...
                            @if(count($subCategories)>0)
                                @foreach ($subCategories as $index => $sub)
                                    <tr>
                                        ...
                                        <td>
                                        @php
                                            $imagePath = public_path($sub->image);
                                        @endphp

                                        @if($sub->image && file_exists($imagePath))
                                            <img 
                                                src="{{ asset($sub->image) }}" 
                                                alt="{{ $sub->name }}" 
                                                width="100" 
                                        ...
                                        <td>
                                            <a class="btn btn-info btn-icon" href="{{route('admin.sub-categories.edit',$sub->id)}}">
                                                <i class="fa-solid fa-pen-to-square"></i>
                                            </a>
                                            <form action="{{route('admin.sub-categories.destroy',$sub->id)}}" class="d-inline" method="post">
                                                @csrf
                                                @method('DELETE')
